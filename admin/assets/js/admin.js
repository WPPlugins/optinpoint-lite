...
"use strict";angular.module("minicolors",[]),angular.module("minicolors").provider("minicolors",function(){this.defaults={theme:"bootstrap",position:"bottom left",defaultValue:"",animationSpeed:50,animationEasing:"swing",change:null,changeDelay:0,control:"hue",hide:null,hideSpeed:100,inline:!1,letterCase:"lowercase",opacity:!1,show:null,showSpeed:100},this.$get=function(){return this}}),angular.module("minicolors").directive("minicolors",["minicolors","$timeout",function(i,n){return{require:"?ngModel",restrict:"A",priority:1,link:function(o,e,r,t){var l=!1,a=function(){var n=angular.extend({},i.defaults,o.$eval(r.minicolors));return n};t.$render=function(){n(function(){var i=t.$viewValue;e.minicolors("value",i)},0,!1)};var u=function(){if(t){var i=a();return i.change=function(i){o.$apply(function(){t.$setViewValue(i)})},e.hasClass("minicolors")&&e.minicolors("destroy"),e.minicolors(i),l?void 0:(n(function(){var i=t.$viewValue;e.minicolors("value",i)},0),void(l=!0))}};u(),o.$watch(a,u,!0)}}}]);
	...
	optpoint.controller("optpointctlr", function($scope,$rootScope,$route,$filter,$compile,$http) {
		$scope.$route = $route;
	...
	optpoint.controller("optpointthe", function($scope,$sce,$filter) {
		...
		$scope.theme_ctrl = {
			...
			edit: function(id){
				...
				$scope.theme = $scope.data.themes[id].options;
				//refresh
				if(isset(msg_edit)){
	...
	optpoint.controller("optpointgen", function($scope,$sce,$filter,$http) {
				...
				aw: {name:'Aweber',
					func:[
						function(){
							...
							$scope.remote.lock();
							$http.post(optinpint_script.ajaxurl+'?action=optinpoint_mailserv_connect&step=0&optinpont_lite_service=1',$scope.mailserv.config)
								.then(function (a) {
		...
		$scope.form={
			...
			edit:function(i){
				$scope.form.step=1;
				$scope.form.ed = i;
				$scope.form.tform = angular.copy($scope.data.cforms[i]);
				$scope.field.ed = -1;
				$scope.edform.load();
			}
		};
		$scope.edform={
			...
			getlist:function(){
				$http.post(optinpint_script.ajaxurl+'?action=optinpoint_lite_mailserv_getlists&optinpont_lite_service=1',$scope.data.mailserv)
					.then(function (a) {
						if(a.status == 200 && typeof a.data != "string"){
							if(a.data.length == 0){
			...
			getfield:function(){
				...
				$http.post(optinpint_script.ajaxurl+'?action=optinpoint_lite_mailserv_getfields&optinpont_lite_service=1',opts)
					.then(function (a) {
						if(a.status == 200 && typeof a.data != "string"){
		...
		$scope.field={
			...
			ok:function(){
				$scope.data.cforms[$scope.form.ed] = angular.copy($scope.form.tform);
				$scope.form.step = 1;
				$scope.form.tform = {};
			},
			cancel:function(){
			...
			extra:{
				...
				add:function(i){
					...
					$scope.form.tform.fields[i].extras.push({"name":"New Option"});
				},
				del:function(i,j){
		...
		$scope.usync={
			...
			add:function(emails,form){
				...
				if($scope.usync.stack > $scope.usync.needle && $scope.usync.cur < $scope.usync.tot){
					$scope.usync.needle++;
					$http.post(optinpint_script.ajaxurl+'?action=optinpoint_lite_usync_add&optinpont_lite_service=1',{e:emails[$scope.usync.cur],f:form})
						.then(function (a) {
							$scope.usync.needle--;
							$scope.usync.done++;
						},function (a) {
			...
			template: function(elem,attrs){
				return '<div class="optpoint_group optpoint_dropc">'
						...
						+'<div class="optpoint_drop">'
							...
							+'</div>'
							+'<div class="optpoint_drop_body">'
							+'<div ng-repeat="(k,v) in ngList" ng-click="sel('+attrs.optclk+')">{{'+attrs.optlname+'}}</div>'
							+'</div>'
						+'</div>'
					+'</div>'
			},
