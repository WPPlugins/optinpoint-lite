...
if(oppoisset(optinpoint.chkcon.l) || oppoisset(optinpoint.chkcon.a) || oppoisset(optinpoint.chkcon.t) || oppoisset(optinpoint.chkcon.f) || oppoisset(optinpoint.chkcon.w) || oppoisset(optinpoint.chkcon.s) ){
	...
	$('.optinpoint-subs-button').click(function (e){
		...
		$.ajax({
			type:"POST",
			url: optinpoint.ajax_url+'?optinpont_lite_service=1',
			data: SubsForm,
			//dataType: "json",
...
function oppovalidate(f,w){
	var a;
	switch(f){
		case 'email':a=new RegExp(/^(("[\w-\s]+")|([\w-]+(?:\.[\w-]+)*)|("[\w-\s]+")([\w-]+(?:\.[\w-]+)*))(@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$)|(@\[?((25[0-5]\.|2[0-4][0-9]\.|1[0-9]{2}\.|[0-9]{1,2}\.))((25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\.){2}(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[0-9]{1,2})\]?$)/i),e=a.test(w);
			break;
